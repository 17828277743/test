<template>
  <div class="app-container">
    ...
    <el-row :gutter="20" class="header">
      ...
      <el-button type="success" :icon="DocumentAdd" @click="handleDialogValue()" v-if="hasAuth('system:user:add')">
        新增
      </el-button>
      <el-popconfirm title="您确定批量删除这些记录吗？" @confirm="handleDelete(null)">
        <template #reference>
          <el-button type="danger" :disabled="delBtnStatus" :icon="Delete" v-if="hasAuth('system:user:delete')">
            批量删除
          </el-button>
        </template>
      </el-popconfirm>
      ...
      <el-table-column prop="action" label="操作" width="400" fixed="right" align="center">
        <template v-slot="scope">
          <el-button type="primary" :icon="Tools" @click="handleRoleDialogValue(scope.row.id,scope.row.sysRoleList)"
                     v-if="hasAuth('system:user:role')">分配角色
          </el-button>
          ...
          <el-button v-if="scope.row.username!='java1234' && hasAuth('system:user:edit')" type="primary" :icon="Edit"
